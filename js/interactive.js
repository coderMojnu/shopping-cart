...
//     document.getElementById('total-phone-prize').innerText = totalPhonePrize;
// });
function handleProductChange(id, isIncrease){
    const productQuantity =  document.getElementById(id);
    const productValue = parseInt(productQuantity.value);
    let productNewQuantity = 0;
    if(isIncrease == true){
        productNewQuantity = productValue + 1;
    }
    if(isIncrease == false && productValue > 0){
        productNewQuantity = productValue - 1;
    }
    productQuantity.value = productNewQuantity;
    const totalProductPrize = productNewQuantity*1219;
    document.getElementById('total-product-prize').innerText = totalProductPrize;
}
